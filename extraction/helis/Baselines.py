...
class MembershipEvaluator(Evaluator):
    ...
    def evaluate(self, model, eva_samples):
        ...
        for k, sample in enumerate(eva_samples):
            ...
            if rank_non > rank:
                csv_file_path = f'helis_{FLAGS.embedding_type}.csv'
                ...
                DLcount += 1
                data.append({'Individual': individual.split(('#'))[1], 'Predicted': predicted_inf, 'Predicted_rank': predicted_inf_rank, 'True': sorted_classes_non[rank_non-1].split(('#'))[1], "True_rank" : rank_non, 'Score_predict': score[predicted_inf_rank-1], 'Score_true': score[rank_non-1], 'Dif' : rank_non - predicted_inf_rank})
        ...
        print('Testing (Inference checking), MRR: %.3f, Hits@1: %.3f, Hits@5: %.3f, Hits@10: %.3f\n\n' % (e_MRR, hits1, hits5, hits10))
